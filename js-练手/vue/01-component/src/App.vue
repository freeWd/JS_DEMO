<template>
  <div id="app">
    ...
    <ShoppingCar :shoppingCarList="shoppingCarList" @removeCarItem="removeCarItem"></ShoppingCar>
  </div>
</template>
...
export default {
  ...
  methods: {
    addCourseList() {
      const hasExist = this.courseList.find(item => item.name === this.courseInfo.name);
      if (!hasExist) {
    ...
    addToShoppingCar(index) {
      ...
      const hasExist = this.shoppingCarList.find(item => item.name === selectCourse.name);
      if (hasExist) {
        hasExist.count ++;
      } else {
